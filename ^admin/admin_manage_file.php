<?php
  ...
  if(isset($_POST['cari'])) 
  {
    ...
    $data_mapel = select("SELECT * FROM dftr_agnd WHERE file like '%".$kata_cari."%' ");
  } else {
    $data_mapel = select("SELECT dftr_agnd.*, guru.id_guru, guru.nama FROM dftr_agnd");
  }
  ...
  if (isset($_POST['tambah'])) {
    if (tambah_data_mapel($_POST) > 0) {
      echo "<script>
              ...
              document.location.href = 'admin_manage_file.php';
            </script>";
    } else {
        echo "<script>
  ...
  if (isset($_POST['hapus'])) {
    if (hapus_data_mapel($_POST) > 0) {
        echo "<script>
                alert('Data File Berhasil Dih apus');
                document.location.href = 'admin_manage_file.php';
              </script>";
    } else {
        echo "<script>
